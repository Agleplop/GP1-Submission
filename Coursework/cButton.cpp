...
void cButton::update(float deltaTime)
{
	...
	if (m_InputMgr->getLeftMouseBtn())
	{
		...
		areaClicked.x = m_InputMgr->getMouseXPos();
		areaClicked.y = m_InputMgr->getMouseYPos();
...
bool cButton::startGame()
{
	//due to mouse controls not working
	//the button was made to respond to
	//the key A being pressed
	if (m_InputMgr->isKeyDown('A'))
		return true;
	else
		return false;
}
bool cButton::exitGame()
{
	//due to mouse controls not working
	//the button was made to respond to
	//the key Z being pressed
	if (m_InputMgr->isKeyDown('X'))
		return true;
	else
		return false;
}
